  ...
  function renderSalesSign(vehicle, dealer, phone) {
    ...
    win.document.write(`<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Verkaufsschild · ${safe(vehicle.brand, '')} ${safe(vehicle.model, '')}</title><style>
      *{box-sizing:border-box} @page{size:A4;margin:0} body{margin:0;background:#eef2f7;font-family:Arial,Helvetica,sans-serif;color:#12294a}
      ...
      .headline{margin:12mm 0 2mm;font-size:12px;letter-spacing:.13em;color:#1687ee;font-weight:800;text-transform:uppercase}.model{font-size:36px;line-height:1.06;margin:0 0 6mm;color:#102a50}.price{font-size:50px;line-height:1;font-weight:950;color:#102a50;margin-bottom:10mm}
      .facts{display:grid;grid-template-columns:repeat(3,1fr);gap:3mm;margin-bottom:9mm}.fact{background:#f3f6fa;border-radius:3mm;padding:4.5mm}.fact span{display:block;color:#6b7a8e;font-size:10px;text-transform:uppercase;letter-spacing:.04em;margin-bottom:1.5mm}.fact b{font-size:17px;line-height:1.25;color:#142c4d}
      .equipment-title{font-size:18px;margin:0 0 4mm}.features{display:grid;grid-template-columns:1fr 1fr;gap:2.4mm 7mm;font-size:15px;line-height:1.35}.features div:before{content:'✓';color:#1687ee;font-weight:900;margin-right:7px}.features .empty:before{content:'';margin:0}
      .bottom{margin-top:auto;padding-top:8mm;border-top:1px solid #dde5ee;display:flex;justify-content:space-between;gap:8mm;color:#6a788b;font-size:10px;line-height:1.4}.bottom strong{color:#314762}
      ...
      @media screen and (max-width:900px){.toolbar,.sheet{width:100%;margin-left:0;margin-right:0}.sheet{min-height:auto;padding:22px}.facts{grid-template-columns:1fr 1fr}.model{font-size:30px}.price{font-size:42px}}
      ...
      <section class="facts">
        <div class="fact"><span>Kilometerstand</span><b>${formatNumber(vehicle.mileage)} km</b></div>
        <div class="fact"><span>Leistung</span><b>${safe(vehicle.hp || '—')} PS / ${safe(vehicle.kw || '—')} kW</b></div>
        <div class="fact"><span>Erstzulassung</span><b>${safe(formatMonth(vehicle.firstRegistration))}</b></div>
        <div class="fact"><span>Kraftstoff</span><b>${safe(vehicle.fuel)}</b></div>
        <div class="fact"><span>Getriebe</span><b>${safe(vehicle.transmission)}</b></div>
        <div class="fact"><span>HU gültig bis</span><b>${safe(formatMonth(vehicle.inspection))}</b></div>
        <div class="fact"><span>Farbe</span><b>${safe(vehicle.color)}</b></div>
        <div class="fact"><span>Fahrzeughalter</span><b>${safe(vehicle.owners || '—')}</b></div>
        <div class="fact"><span>Kennzeichen</span><b>${safe(vehicle.plate)}</b></div>
      </section>
      ...
      <footer class="bottom"><div><strong>${safe(dealer.company, 'Autohaus')}</strong>${dealerAddress ? `<br>${dealerAddress}` : ''}</div><div>FIN: ${safe(vehicle.vin)}<br>Stand: ${new Intl.DateTimeFormat('de-DE').format(new Date())}</div></footer>
    </main></body></html>`);
    win.document.close();
  }
